...
function LettersFromGrid()
{
	var alignments = ["top left", "top center", "top right", "center left", "center", "center right", "bottom left", "bottom center", "bottom right"];

	var dialogXML = ''
	dialogXML += '<label width="300" value="The document\'s width and height are divided by the values below." />';
	...
	dialogXML += '<label width="300" value="Starting Count: (Hexadecimal)" />';
		...
		for (var i = 0; i < dialogData.vdiv; i++)
		{
			for (var j = 0; j < dialogData.hdiv; j++)
			{
				...
				for each(var selectedItem in doc.selection)
				{
					...
					var itemIndex = parseInt(dialogData.startcount, 16) + a;
