...
class BAlphabetDebug extends MusicBeatState
{
    ...
    public function create():Void
    {
        ...
        normalText = new BAlphabet(0, 0, "", { font: BAlphabetData.uniqueFonts[currentFont] });
        add(normalText);

        typedText = new BAlphabetTyped(0, -900, "<b>Typed text testing,</b> <c=00FFAA>make sure</c> <i>this is</i> <s=0.5>a</s> <a=0.5>really</a> <W>long string</W> <S>to test it properly!</S>");
        add(typedText);
        ...
        // typedText.resetOrigin();

        for (i in 0...3)
        {
            var alignmentLine = new FunkinSprite(0, -225 + (i * 26));
            alignmentLine.makeSolidColor(300, 2, 0x7F000000);
            add(alignmentLine);
            offsetLines.push(alignmentLine);
        }

        offsetText = new BAlphabet(0, -225, "i!Aa", { font: BAlphabetData.uniqueFonts[currentFont] });
        // offsetText.alignment = "center";
        add(offsetText);

        // var test01:BAlphabet = new BAlphabet(0, -350, "<c=FF0000>HUNDREC</c>", { font: "example" });
        // add(test01);
        // var test02:BAlphabet = new BAlphabet(-10, -350, "<c=00FF00>ONEDREC</c>", { font: "example" });
        ...
        fontName = new FlxText(500, -200, 0, "aaa", 32);
        fontName.text = "Current font: " + BAlphabetData.uniqueFonts[currentFont];
        add(fontName);

        typefaceName = new FlxText(500, -150, 0, "aaa", 32);
        add(typefaceName);

        glyphCount = new FlxText(500, -100, 0, "aaa", 32);
        add(glyphCount);

        var unicodeTest = new BAlphabet(-100, 0, "&#65;&#x42;&#0067;&#x00044; &#x2190; ABCD?", { alignment: "right", baseColor: "282828" });
        add(unicodeTest);

        var test03:BAlphabet = new BAlphabet(-100, -32, "This is a tiny Hello World!", { font: "pixel-example", alignment: "right" });
        add(test03);

        regenGlyphs(BAlphabetData.uniqueTypefaces.indexOf("latin"));
    ...
    public function update(elapsed:Float):Void
    {
        ...
        if (FlxG.keys.justPressed.U)
        {
            ...
            allLetterIndexes.sort(sortOrder);
            fontName.text = "Current font: " + BAlphabetData.uniqueFonts[currentFont];
            currentTypeface = BAlphabetData.uniqueTypefaces.indexOf("latin");
            changedFont = true;
            regenGlyphs(0);
        }
    ...
    function regenGlyphs(change:Int = 0):Void
    {
        ...
        if (changedFont)
        {
            ...
            var letterHeight = BAlphabetData.allFonts.get(BAlphabetData.uniqueFonts[currentFont]).height;
            for (i in 0...3)
            {
                offsetLines[i].y = -225 + letterHeight * (i / 2) - i;
            }
    ...
    public function changeOffsetCharacter(change:Int = 0):Void
    {
        ...
        offsettedLetter.offset.set(0, 0);
        modifiedOffsets = [0, 0];
    ...
    public function changeOffsetOffset(changeX:Int = 0, changeY:Int = 0):Void
    {
        ...
        var offsettedLetter = offsetText.letters[1];
        offsettedLetter.offset.set(modifiedOffsets[0], modifiedOffsets[1]);
        updateOffsetInfo();
    }
    ...
    public function updateOffsetInfo():Void
    {
        ...
        offsetInfo.text += "x: " + (letterOffsets[0]) + " (" + -modifiedOffsets[0] + ")\n";
        offsetInfo.text += "y: " + (letterOffsets[1]) + " (" + -modifiedOffsets[1] + ")\n";
